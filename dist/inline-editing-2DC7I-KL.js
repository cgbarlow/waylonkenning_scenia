const e = `# Inline Editing

![Inline editing](../../public/features/inline-editing.png)

## Editing a cell

Double-click any cell to open it for editing. Text fields, number fields, dropdowns, and checkboxes are all supported.

Edits are saved immediately when you click away or press Enter — there is no Save button. Changes are reflected on the timeline in real time.

## Adding a row

Click **Add Row** at the bottom of any table. A new row with a unique auto-generated ID is created. Fill in the fields inline.

## Deleting a row

Click the trash icon on the row. A confirmation modal appears before the row is permanently removed.

Note: deleting an asset also removes all initiatives associated with it (cascading delete).

## Resetting data

- **Reset — use demo data**: replaces all data with the built-in demo dataset. Useful for exploring features or recovering a clean starting point.
- **Reset — delete all data**: clears all tables completely.

Both actions show a confirmation modal before proceeding.

---

- Previous: [Data Manager Overview](overview.md)
- Next: [CSV Paste](csv-paste.md)
`;
export {
  e as default
};
